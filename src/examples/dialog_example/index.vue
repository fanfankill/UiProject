<template>
  <div>
    ...
    <showcomponent title="基础用法" subtitle="对话框的基础用法" ctname="Dialog" :codes="code1">
      ...
      <template v-slot:desc>
        ...
        分为两个部分：<code>body</code>和<code>footer</code>，<code>footer</code>
    ...
    <showcomponent title="嵌套的对话框" subtitle="对话框的嵌套用法" ctname="Dialog" :codes="code2">
      <template v-slot:showarea>
        <ct-button @click="outerVisible = true">点击打开外层对话框</ct-button>
      </template>
      <template v-slot:desc>
        正常情况下，我们不建议使用嵌套的 Dialog，如果需要在页面上同时显示多个 Dialog，可以将它们平级放置。
      </template>
    </showcomponent>
    ...
    <showcomponent title="居中布局" subtitle="标题和底部可居中布局" ctname="Dialog" :codes="code3">
      <template v-slot:showarea>
        <ct-button @click="centerVisible = true">点击打开对话框</ct-button>
      </template>
      <template v-slot:desc>
        将 <code>center</code> 设置为 <code>true</code> 即可使标题和底部居中。<code>center</code>仅影响标题和底部区域。Dialog 的内容是任意的，在一些情况下，内容并不适合居中布局。如果需要内容也水平居中，请自行为其添加 CSS
      </template>
    </showcomponent>
    <ct-dialog title="标题居中" :visible.sync="centerVisible" width="30%" center>
...
export default {
  ...
  data() {
    return {
      ...
      outerVisible: false,
      centerVisible: false,
      code1: `<ct-button @click="dialogVisible = true">点击打开对话框</ct-button>
