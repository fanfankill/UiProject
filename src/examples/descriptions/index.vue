<template>
  <div>
        ...
        <div class="block_show_div">
          <div class="flex_col">
            <ct-descriptions  title="User Info" column="3">
              <ct-descriptions-item label="Username">{{info.name}}</ct-descriptions-item>
              <ct-descriptions-item label="Age"><code>{{ info.age }}</code></ct-descriptions-item>
              <ct-descriptions-item label="Gender">{{ info.sex }}</ct-descriptions-item>
              <ct-descriptions-item label="School">{{ info.school }}</ct-descriptions-item>
              <ct-descriptions-item label="Profession">{{ info.major }}</ct-descriptions-item>
              <ct-descriptions-item label="Hobby">{{ info.hobby }}</ct-descriptions-item>
              <ct-descriptions-item label="Telephone">{{ info.phone }}</ct-descriptions-item>
              <ct-descriptions-item label="Address">{{ info.address }}</ct-descriptions-item>
            </ct-descriptions>
          </div>
        </div>
      </template>
      <template v-slot:desc>
        使用<code>title</code>,<code>column</code>，<code>label</code>属性来定义 Descriptions 的样式
      </template>
    </showcomponent>
    <showcomponent
        title="不同尺寸"
    ...
    >
      ...
      <template v-slot:desc>
        使用<code>size</code>属性来定义 Descriptions 的样式
      </template>
    </showcomponent>
    <showcomponent
        title="有无边框"
    ...
    >
      ...
      <template v-slot:desc>
        使用<code>borderless</code>属性来定义 Descriptions 的样式
      </template>
    </showcomponent>
    <showcomponent
        title="垂直列表"
    ...
    >
      <template v-slot:showarea>
        <div class="block_show_div">
          <div class="flex_col">
            <ct-row>
              <ct-radio v-model="radio3" label="large">Large</ct-radio>
              <ct-radio v-model="radio3" label="medium">Default</ct-radio>
              <ct-radio v-model="radio3" label="small">Small</ct-radio>
            </ct-row>
            <ct-descriptions direction="vertical" :size="radio3" title="User info" column="3">
              <ct-descriptions-item label="Username">{{info.name}}</ct-descriptions-item>
              <ct-descriptions-item label="Age"><code>{{ info.age }}</code></ct-descriptions-item>
              <ct-descriptions-item label="Gender">{{ info.sex }}</ct-descriptions-item>
              <ct-descriptions-item label="School">{{ info.school }}</ct-descriptions-item>
...
export default {
  ...
  data () {
    return {
      code1:`<ct-descriptions title="User Info" column="3">
...
`,
      info: {
        ...
        school: 'Scut',
        major: 'computer',
        address: 'No.1188, Xiaoguwei Avenue, Xiaoguwei District, Guangzhou, Guangdong Province',
        hobby: 'Movies',
        phone: 18888888888,
      ...
      parameter:{
        ...
        contents:[
          ['borderless','是否不带边框','Boolean','true/false','false'],
          ['column','列数','Number','','3'],
          ['direction','排列的方向','String','vertical/horizontal','horizontal'],
          ['size','列表的尺寸','String','large/default/small','default(medium)'],
